...
api_hash = 'API_HASH'
phone = 'PHONENUMBER'
ADMIN_TAG = 'Your_Username OR ID БЕЗ @'
MY_ID = Your_ID
...
async def get_messages(channel_link, limit=None):
    ...
    try:
        ...
        is_broadcast = main_entity.type == enums.ChatType.CHANNEL
            ...
            async for m in client.get_chat_history(chat_obj.id, limit=limit):
                if m.text or m.caption:
                    ...
                    print(f"{colorama.Fore.WHITE}[{colorama.Fore.BLUE}{time.asctime()}{colorama.Fore.WHITE}] {colorama.Fore.WHITE}[{colorama.Fore.YELLOW}Message{colorama.Fore.WHITE}] "
                          f"{colorama.Fore.GREEN}{text.strip()[:90]}... "
        ...
        if full.linked_chat:
            linked = full.linked_chat
            all_data["linked"] = linked
            await smart_join(linked.id)
...
async def func(args):
    if args.c:
        ...
        # КАСКАДНЫЙ СБОР
        try:
            # 1. Прямой список участников
            async for member in client.get_chat_members(target.id):
                u = member.user
                if u.id != MY_ID:
                    final_users[u.id] = {"name": f"{u.first_name or ''} {u.last_name or ''}".strip(), "user": u.username or "Unknown", "phone": u.phone_number or "Unknown"}
                        ...
                        for r in res.users:
                            if r.id not in final_users and r.id != MY_ID:
                                final_users[r.id] = {
                                    ...
                                    "user": r.username or "Unknown", "phone": r.phone or "Unknown"}
                    except: pass
            
            # --- [МЕТОД 5: ПАРСИНГ СКРЫТЫХ ID ЧЕРЕЗ ОТВЕТЫ] ---
            #print(f"{colorama.Fore.CYAN}[*] Запуск сбора через Reply-метаданные...")
            async for m in client.get_chat_history(target.id, limit=500):
                if m.reply_to_message:
                    ...
                    if u and u.id not in final_users and u.id != MY_ID:
                        ...
                        print(f"{colorama.Fore.WHITE}[REPLY_FIND] {colorama.Fore.MAGENTA}ID: {u.id} {colorama.Fore.GREEN}User: {u.first_name}")
        except: pass
